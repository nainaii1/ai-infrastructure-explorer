...
digest of one analyst's (@aleabitoreddit) views, and a **Desk layer** — Codex's
...
theses → conviction tiers → Codex's desk verdicts → operator decision.
...
- **v5 — Conviction tiers + Desk verdicts** ✅ done (2026-07-03). Every ticker
  ...
  **Watchlist** opens focused on **Core** only (it's the decision surface —
  ...
  On top sits the **Desk** layer: `ingest/store/verdicts.json` holds Codex's
  ...
  (`.Codex/skills/weekly-review/SKILL.md`) in a Codex session — no
...
- **Live counts** (approximate, check `ingest/store/*.json` for current):
  ...
  refreshed 2026-07-03 manually via Codex per the v4 workaround above).
- **Signal Digest** 📋 designed, not built. A planned feature — periodic,
  Codex-authored per-ticker digests (AI Signal Watch style: short overview +
...
- `desk` → Codex's weekly per-ticker verdicts (`store/verdicts.json`,
...
├── AGENTS.md                   this file — build spec + current status
├── README.md                   quick start + project map
├── .Codex/skills/weekly-review/SKILL.md   the /weekly-review desk procedure
├── docs/
...
└── ingest/                     THE BACKEND — Python tooling that regenerates data.js
    ...
    ├── synthesize.py            the "Brain" — Codex-synthesized theme digests (needs ANTHROPIC_API_KEY)
