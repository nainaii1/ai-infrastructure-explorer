...
This module defines what a seat IS and what a finding must satisfy to be
recorded at all. Running a pass over a shortlist — choosing the names, calling
the seats, merging the findings back — lives in pre_review.py, which was split
out on 2026-07-27 when this file passed the ~350-line threshold the plan set.

Mirrors ingest/synthesize.py: every function here is PURE. No network, no file
I/O, no API client. The injected `call_fn(system, user) -> dict` that lets a
Claude Code session drive a run without an API key (the operator does not have
one — see CLAUDE.md, v4) is taken by pre_review.run_seats, not here.
...
def coerce_str(value):
    ...
    Public because pre_review reads the same records with the same rule: a
    non-string where a string belongs is treated as absent, never coerced into
    a value that could pass a check. One definition so the two modules cannot
    drift into disagreeing about what "unreadable" means.
    """
...
def validate_finding(raw, seat, ticker, allowed_tickers):
    ...
    finding = coerce_str(raw.get("finding"))
    if not finding:
        return None

    caller_ticker = coerce_str(ticker).upper()
    if caller_ticker not in {t.upper() for t in allowed_tickers}:
        return None

    model_ticker = coerce_str(raw.get("ticker")).upper()
    ...
    direction = coerce_str(raw.get("direction")).lower()
    ...
    confidence = coerce_str(raw.get("confidence")).lower()
